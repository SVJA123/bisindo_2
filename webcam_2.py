    ...
    if results.multi_hand_landmarks:
        for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
            if idx >= 2:  # Limit to 2 hands
                break
            offset = idx * 63  # 63 floats per hand
            coords = []
            for lm in hand_landmarks.landmark:
                coords.extend([lm.x, lm.y, lm.z])
            landmarks[offset:offset + 63] = coords
...
def main():
    ...
    model_path = "hand_gesture_cnn_model.keras"  # Path to your saved model
    ...
    while True:
        ...
        if not np.allclose(landmarks, 0):
            ...
            input_data = landmarks.reshape(1, 21, 6, 1)  # Shape: (1, 21, 6, 1)
