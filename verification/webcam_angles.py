...
from model_angle.normalize import normalize_landmarks
...
def main():
    model_path = "model_angle/hand_gesture_model_angles.keras"  
    ...
    while True:
        ...
        landmarks, angles, annotated_frame = process_frame(frame, hands)

        concatenated = np.concatenate([landmarks, angles])

        if not np.allclose(landmarks, 0):
            input_data = concatenated.reshape(-1, 46, 2, 1)  # Shape: (1, 21, 2, 1)
