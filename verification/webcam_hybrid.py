...
def main():
    ...
    while True:
        ...
        if not np.allclose(landmarks, 0):
            input_landmarks = landmarks.reshape(-1, 42, 2, 1)  # Shape: (1, 42, 2, 1)
            input_angles = angles.reshape(1, 8)
            prediction = model.predict([input_landmarks, input_angles], verbose=0)
